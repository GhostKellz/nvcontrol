...
For newer open-driver troubleshooting, use:
...
- running kernel vs module target mismatch
- userspace driver vs kernel module mismatch
- firmware layout/path/file visibility
- expected firmware search paths
- Arch package presence/version hints for mixed states
- firmware path ownership verification on Arch

If you need to share the full state, generate a support bundle:

```bash
nvctl driver support-bundle --output ~/.local/state/nvcontrol/support/gsp-support.txt
```
