...
Navigate to the **Drivers** area in the TUI for ASUS power-monitor status.

## GUI

Open the dedicated **Power** tab to see:
- real-time GPU power draw
- connector health
- per-rail current estimates
- rolling averages and peaks
- warning/sample counts
- I2C bus visibility
...
If nvcontrol detects the Astral model but reports that Power Detector+ is unavailable, verify `i2c-tools`, `i2c-dev`, and `i2c-nvidia_gpu` access before assuming the board is unsupported.

## More Information

- [power-detection.md](power-detection.md) - Technical details
- [asus-astral.md](asus-astral.md) - Full feature list
- [rtx-5090-setup.md](rtx-5090-setup.md) - RTX 50 setup
