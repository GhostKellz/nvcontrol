...
fn enable_gpu_scheduling() -> NvResult<bool> {
    ...
    unsafe { std::env::set_var("__GL_THREADED_OPTIMIZATIONS", "1"); }
    unsafe { std::env::set_var("__GL_SYNC_TO_VBLANK", "0"); }
...
fn disable_cpu_scaling() -> NvResult<bool> {
    ...
    let result = Command::new("sudo")
        .args(&["tee", "/sys/devices/system/cpu/cpufreq/policy*/scaling_max_freq"])
...
fn optimize_memory_latency() -> NvResult<bool> {
