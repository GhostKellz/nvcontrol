                ...
                for entry in entries.flatten() {
                    ...
                    if path.is_dir() {
                        ...
                        if name.contains("shader") || name.contains("cache") {
                            if let Ok(info) = get_cache_info(&path, ShaderCacheType::Game(name.to_string())) {
                                caches.push(info);
                            }
                        }
                    }
...
fn get_cache_info(path: &Path, cache_type: ShaderCacheType) -> NvResult<ShaderCacheInfo> {
    if !path.exists() {
        return Err(NvControlError::DisplayDetectionFailed("Cache path does not exist".to_string()));
    }
    
    let metadata = fs::metadata(path)
        .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to read cache metadata: {}", e)))?;
...
fn calculate_directory_size(path: &Path) -> NvResult<(u64, usize)> {
    ...
    if path.is_file() {
        let metadata = fs::metadata(path)
            .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to read file metadata: {}", e)))?;
        return Ok((metadata.len(), 1));
    }
    
    let entries = fs::read_dir(path)
        .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to read directory: {}", e)))?;
    
    for entry in entries {
        let entry = entry
            .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to read directory entry: {}", e)))?;
...
pub fn clear_shader_cache() -> NvResult<()> {
    ...
    println!("\nCleared {} shader caches, freed {:.1} MB", cleared_count, total_size_cleared);
    Ok(())
}
...
pub fn clear_shader_cache_by_type(cache_type: ShaderCacheType) -> NvResult<()> {
    ...
    if cleared_count == 0 {
        ...
        println!("Cleared {} {} caches, freed {:.1} MB", cleared_count, cache_type.as_str(), total_size_cleared);
    }
...
fn clear_cache_directory(path: &Path) -> NvResult<()> {
    ...
    if path.is_file() {
        fs::remove_file(path)
            .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to remove file: {}", e)))?;
    } else {
        ...
        for entry in entries {
            let entry = entry
                .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to read entry: {}", e)))?;
            
            let entry_path = entry.path();
            if entry_path.is_dir() {
                fs::remove_dir_all(&entry_path)
                    .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to remove directory: {}", e)))?;
            } else {
...
pub fn precompile_shaders(game_path: &str) -> NvResult<()> {
    ...
    if let Some(app_id) = extract_steam_app_id(game_path) {
        println!("Triggering Steam shader pre-compilation for app ID: {}", app_id);
        ...
        match output {
            ...
            Err(_) => {
                ...
                match output {
                    Ok(_) => println!("Steamcmd validation triggered (includes shader compilation)"),
                    Err(_) => println!("Steam not available, manual shader compilation not possible"),
                }
            }
        }
    } else {
                ...
                if file_name_str.starts_with("appmanifest_") && file_name_str.ends_with(".acf") {
                        ...
                        .and_then(|s| s.strip_suffix(".acf"))
                    {
                        ...
                        if let Ok(content) = fs::read_to_string(entry.path()) {
                            if content.contains(&game_path.split('/').last().unwrap_or("")) {
...
pub fn optimize_shader_compilation() -> NvResult<()> {
    ...
    unsafe { std::env::set_var("__GL_SHADER_DISK_CACHE_PATH", "/tmp/nvidia-shader-cache"); }
    unsafe { std::env::set_var("__GL_SHADER_DISK_CACHE_SIZE", "1073741824"); } // 1GB
    
    // Create shader cache directory with proper permissions
    let cache_dir = PathBuf::from("/tmp/nvidia-shader-cache");
    if !cache_dir.exists() {
        fs::create_dir_all(&cache_dir)
            .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to create cache directory: {}", e)))?;
        ...
        #[cfg(unix)]
        {
            ...
            fs::set_permissions(&cache_dir, permissions)
                .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to set permissions: {}", e)))?;
        }
    }
