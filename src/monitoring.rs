...
pub fn export_gpu_metrics(
    ...
    println!(
        "📊 Collecting GPU metrics for {} seconds...",
        duration_seconds
    );
...
pub fn run_gpu_benchmark(
    ...
    while start_time.elapsed().as_secs() < duration_seconds as u64 {
        if let Ok(snapshot) = collect_device_metrics(backend, 0) {
            ...
            if sample_count % 10 == 0 {
                println!(
                    "Sample {}: GPU {}%, Memory {}%, Temp {:.1}°C, Power {:.1}W",
                    sample_count + 1,
    ...
    Command::new("yes")
        ...
        .map_err(|e| {
            NvControlError::DisplayDetectionFailed(format!(
                "Failed to start fallback stress: {}",
                e
            ))
        })
}
...
fn analyze_benchmark_results(
    ...
    let graphics_score = if test_type == "graphics" {
        compute_score
    } else {
        None
    };
