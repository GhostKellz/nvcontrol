...
pub fn get_gamescope_version() -> NvResult<String> {
    let output = Command::new("gamescope")
        .arg("--version")
        .output()
        .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to get Gamescope version: {}", e)))?;
    
    if output.status.success() {
        let version = String::from_utf8_lossy(&output.stdout).trim().to_string();
        Ok(version)
    } else {
        Err(NvControlError::DisplayDetectionFailed("Gamescope not available".to_string()))
    }
}
...
pub fn launch_with_gamescope_config(game_path: &str, config: &GamescopeConfig) -> NvResult<()> {
    ...
    if !Path::new(game_path).exists() {
        return Err(NvControlError::DisplayDetectionFailed(
            format!("Game not found: {}", game_path)
        ));
    }
    ...
    let mut child = cmd
        ...
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to launch Gamescope: {}", e)))?;
    
    println!("Gamescope launched with PID: {}", child.id());
    
    // Wait for the process to complete
    let exit_status = child.wait()
        .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Error waiting for Gamescope: {}", e)))?;
...
pub fn create_steam_gamescope_wrapper(game_name: &str, game_path: &str, preset: GamescopePreset) -> NvResult<PathBuf> {
    let config = preset.to_config();
    ...
    let user_dirs = directories::UserDirs::new()
        .ok_or_else(|| NvControlError::DisplayDetectionFailed("Could not find home directory".to_string()))?;
    let home_dir = user_dirs.home_dir();
    
    let local_bin = home_dir.join(".local/bin");
    fs::create_dir_all(&local_bin)
        .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to create local bin directory: {}", e)))?;
    
    let script_path = local_bin.join(format!("gamescope-{}.sh", game_name.to_lowercase().replace(' ', "-")));
    
    fs::write(&script_path, script_content)
        .map_err(|e| NvControlError::DisplayDetectionFailed(format!("Failed to write script: {}", e)))?;
...
fn create_gamescope_config_file() -> NvResult<()> {
    let user_dirs = directories::UserDirs::new()
        .ok_or_else(|| crate::NvControlError::DisplayDetectionFailed("Could not find home directory".to_string()))?;
...
"#;
    ...
    std::fs::create_dir_all(&config_dir)
        .map_err(|e| crate::NvControlError::DisplayDetectionFailed(format!("Failed to create config directory: {}", e)))?;
    
    let config_file = config_dir.join("nvcontrol-gamescope.conf");
    std::fs::write(&config_file, config_content)
        .map_err(|e| crate::NvControlError::DisplayDetectionFailed(format!("Failed to write config file: {}", e)))?;
