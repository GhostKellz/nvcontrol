...
pub fn get_display_info() {
    ...
    for display in list_displays() {
        ...
        println!("    HDR Capable: {}", if display.hdr_capable { "Yes" } else { "No" });
        if display.hdr_capable {
            println!("    HDR Status: {}", if display.hdr_enabled { "Enabled" } else { "Disabled" });
        ...
        && output.status.success() {
            ...
            for (id, line) in output_str.lines().enumerate() {
                if !line.starts_with(' ') && !line.is_empty() {
                    ...
                    let hdr_enabled = if hdr_capable { get_hdr_status(&name) } else { false };
...
pub fn toggle_hdr(display_id: usize) -> NvResult<bool> {
    ...
    let display = displays.get(display_id)
        .ok_or_else(|| NvControlError::DisplayDetectionFailed(format!("Display {display_id} not found")))?;
...
fn toggle_hdr_kde(display_name: &str, enable: bool) -> NvResult<()> {
    ...
    let output = std::process::Command::new("kscreen-doctor")
        ...
        .output()
        .map_err(|e| NvControlError::DisplayDetectionFailed(format!("kscreen-doctor not found: {e}")))?;
    
    if output.status.success() {
        Ok(())
    } else {
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(NvControlError::DisplayDetectionFailed(format!("kscreen-doctor failed: {stderr}")))
    }
}
...
fn toggle_hdr_gnome(display_name: &str, enable: bool) -> NvResult<()> {
    ...
    let output = std::process::Command::new("busctl")
        .args(["call", "org.gnome.Mutter.DisplayConfig", 
               "/org/gnome/Mutter/DisplayConfig",
               "org.gnome.Mutter.DisplayConfig", method, "s", display_name])
        .output();
    
    match output {
        Ok(result) if result.status.success() => Ok(()),
        _ => Err(NvControlError::DisplayDetectionFailed(
            "GNOME HDR not supported. Requires GNOME 46+ with experimental features enabled.".to_string()
        ))
    }
}
...
fn toggle_hdr_hyprland(display_name: &str, enable: bool) -> NvResult<()> {
    ...
    if output.status.success() {
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(NvControlError::DisplayDetectionFailed(format!("hyprctl failed: {stderr}")))
    }
}
...
fn toggle_hdr_sway(display_name: &str, enable: bool) -> NvResult<()> {
    ...
    if output.status.success() {
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(NvControlError::DisplayDetectionFailed(format!("swaymsg failed: {stderr}")))
    }
}
...
pub fn list_icc_profiles() -> Vec<String> {
    ...
    let icc_dirs = vec![
        ...
        directories::UserDirs::new().map(|d| d.home_dir().join(".color/icc")).unwrap_or_default(),
        directories::UserDirs::new().map(|d| d.home_dir().join(".icc")).unwrap_or_default(),
    ];
...
pub fn load_icc_profile(display_id: usize, profile_name: &str) -> NvResult<()> {
    ...
    let profile_path = find_icc_profile_path(profile_name)
        .ok_or_else(|| NvControlError::DisplayDetectionFailed(format!("ICC profile {profile_name} not found")))?;
...
pub fn open_icc_folder() -> NvResult<()> {
    ...
    Err(NvControlError::DisplayDetectionFailed(
        format!("Could not open ICC folder: {}. No suitable file manager found.", icc_dir.display())
    ))
}
